...
class FloorLimitTimeWindow(ui.ScriptWindow):
	...
	def PressJumpButton(self):
		...
		self.btnJump.Hide()
		m2netm2g.SendChatPacket("/jumpfloor")
		
	def NextMission(self):
		m2netm2g.SendChatPacket("/nextfloor") # 수정이 필요함.
		self.btnJump.Hide()
...
class Reward12ziWindow(ui.ScriptWindow):
	...
	def PressButton(self, type):				
		...
		m2netm2g.SendChatPacket("/cz_reward %d" % (type))
	...
	def SendClear(self, value):
		...
		m2netm2g.SendChatPacket("/cz_check_box %d %d" % (type, value))
	...
	def Open(self, yellowmark, greenmark, yellowreward, greenreward, goldreward):
		for i in xrange(self.COLUMN_SIZE):
			itemCount = playerm2g2.GetItemCountByVnum(self.COLUMN_ITEM_LIST[i])
		...
		for i in xrange(self.ROW_SIZE):
			itemCount = playerm2g2.GetItemCountByVnum(self.ROW_ITEM_LIST[i])
