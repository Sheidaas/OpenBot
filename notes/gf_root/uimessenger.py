...
class MessengerFriendItem(MessengerMemberItem):
	...
	def OnRemove(self):
		...
		m2netm2g.SendMessengerRemovePacket(self.key, self.name)
		return True
...
class MessengerGuildItem(MessengerMemberItem):
	...
	def OnWarp(self):
		m2netm2g.SendGuildUseSkillPacket(155, self.key)

	def CanRemove(self):
		if app.ENABLE_SECOND_GUILDRENEWAL_SYSTEM:
			if guild.GetEnemyGuildName() != "":
				return False
		else:
	...
	def OnRemove(self):
		m2netm2g.SendGuildRemoveMemberPacket(self.key)
		return True
...
if app.ENABLE_MESSENGER_BLOCK:
	class MessengerBlockFriendItem(MessengerMemberItem):
		...
		def OnRemove(self):
			m2netm2g.SendMessengerBlockRemovePacket(self.key, self.name)
			return True
...
class MessengerWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			if app.ENABLE_MESSENGER_BLOCK:
				self.blockButton = self.GetChild("BlockFrindButton")
	...
	def OnInputMobileMessage(self):
		...
		m2netm2g.SendMobileMessagePacket(self.inputDialog.name, text)
		self.OnCloseInputDialog()
		return True
	...
	def OnAddFriend(self):
		...
		if text:
			m2netm2g.SendMessengerAddByNamePacket(text)
	...
	if app.ENABLE_MESSENGER_BLOCK:
		...
		def OnBlockFriend(self):
			...
			if text:
				m2netm2g.SendMessengerBlockAddByNamePacket(text)
