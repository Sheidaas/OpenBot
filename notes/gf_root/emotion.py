...
def __RegisterSharedEmotionAnis(mode, path):
	chrmgrm2g.SetPathName(path)
	chrmgrm2g.RegisterMotionMode(mode)

	for key, val in ANI_DICT.items():
		if app.ENABLE_12ZI:
			chrmgrm2g.RegisterCacheMotionData(mode, key, val)
		else:
			chrmgrm2g.RegisterMotionData(mode, key, val)
...
def RegisterEmotionAnis(path):
	...
	chrmgrm2g.SetPathName(weddingPath)
	chrmgrm2g.RegisterMotionMode(chr.MOTION_MODE_WEDDING_DRESS)
	if app.ENABLE_12ZI:
		chrmgrm2g.RegisterCacheMotionData(chr.MOTION_MODE_WEDDING_DRESS, chr.MOTION_WAIT, "wait.msa")
		chrmgrm2g.RegisterCacheMotionData(chr.MOTION_MODE_WEDDING_DRESS, chr.MOTION_WALK, "walk.msa")
		chrmgrm2g.RegisterCacheMotionData(chr.MOTION_MODE_WEDDING_DRESS, chr.MOTION_RUN, "walk.msa")
	else:
		chrmgrm2g.RegisterMotionData(chr.MOTION_MODE_WEDDING_DRESS, chr.MOTION_WAIT, "wait.msa")
		chrmgrm2g.RegisterMotionData(chr.MOTION_MODE_WEDDING_DRESS, chr.MOTION_WALK, "walk.msa")
		chrmgrm2g.RegisterMotionData(chr.MOTION_MODE_WEDDING_DRESS, chr.MOTION_RUN, "walk.msa")

def RegisterEmotionIcons():
	for key, val in ICON_DICT.items():
		playerm2g2.RegisterEmotionIcon(key, val)
