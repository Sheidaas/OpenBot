...
class MyCharacters :
	...
	def LoadCharacterData(self) :
		...
		for i in xrange(m2netm2g.CHARACTER_SLOT_COUNT_MAX) :
			pid 			= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_ID) 
			
			if not pid :
				self.EmptySlot.append(i)
				continue
				
			name 			= m2netm2g.GetAccountCharacterSlotDataString(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_NAME)
			level 			= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_LEVEL)
			race 			= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_RACE)
			playtime 		= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_PLAYTIME)
			guildname 		= m2netm2g.GetAccountCharacterSlotDataString(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_GUILD_NAME)
			form 			= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_FORM)
			hair 			= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_HAIR)
			stat_str 		= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_STR)
			stat_dex		= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_DEX)
			stat_hth		= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_HTH)
			stat_int		= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_INT)
			last_playtime	= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_LAST_PLAYTIME)
			change_name		= m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_CHANGE_NAME_FLAG)
			acce = m2netm2g.GetAccountCharacterSlotDataInteger(i, m2netm2g.ACCOUNT_CHARACTER_SLOT_ACCE)
...
class SelectCharacterWindow(ui.Window) :	
	...
	def Open(self):
		...
		##Background##
		self.backGroundDict = {
			m2netm2g.EMPIRE_B : "d:/ymir work/ui/intro/empire/background/empire_chunjo.sub",
			m2netm2g.EMPIRE_C : "d:/ymir work/ui/intro/empire/background/empire_jinno.sub",
		}
		...
		self.flagDict = {
			m2netm2g.EMPIRE_B : "d:/ymir work/ui/intro/empire/empireflag_b.sub",
			m2netm2g.EMPIRE_C : "d:/ymir work/ui/intro/empire/empireflag_c.sub",
		}
		...
		if app.ENABLE_STEAM and app.GetLoginType() == app.LOGIN_TYPE_STEAM:
			...
			self.myID.SetText(m2netm2g.GetLoginID())
		...
		my_empire = m2netm2g.GetEmpireID()
	...
	def SetEmpire(self, empire_id):
		...
		if empire_id != m2netm2g.EMPIRE_A :
	...
	def StartGameButton(self):
		...
		if app.ENABLE_BATTLE_FIELD:
			import playerm2g2
			playerm2g2.SetBattleButtonFlush(True)
	...
	def OnUpdate(self):
		...
		if self.MotionStart and app.GetTime() - self.MotionTime >= 2.0 :
			...
			playTime = m2netm2g.GetAccountCharacterSlotDataInteger(chrSlot, m2netm2g.ACCOUNT_CHARACTER_SLOT_PLAYTIME)
	...
	def AcceptInputPrivateCode(self) :
		...
		pid = m2netm2g.GetAccountCharacterSlotDataInteger(self.RealSlot[self.SelectSlot], m2netm2g.ACCOUNT_CHARACTER_SLOT_ID)
		
		if not pid :
			self.PopupMessage(localeInfo.SELECT_EMPTY_SLOT)
			return

		m2netm2g.SendDestroyCharacterPacket(self.RealSlot[self.SelectSlot], privateCode)
	...
	if app.ENABLE_MONSTER_CARD:
		def SetIllustrationInit(self):
			import playerm2g2
			playerm2g2.SetIllustrationDataLoad(False)
