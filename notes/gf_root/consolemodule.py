...
class Console(object):
	...
	def ReloadDevel(self):
		...
		if app.ENABLE_EXTEND_INVEN_SYSTEM:
			...
			reload(uiNewInventory)
			self.Print("ReloadNewInventory")
	...
	def RegisterCharacterEffect(self, effectType, effectFileName):
		...
		chrmgrm2g.RegisterEffect(chrmgrm2g.EFFECT_STATE+int(effectType), "", effectFullPathFileName)
	...
	def ShowPickedCharacterInfo(self):
		...
		vid=chrmgrm2g.GetPickedVID()
		info=chrmgrm2g.GetVIDInfo(vid)
		self.Print(info)
...
#
	...
	def SetAutoCameraRotationSpeed(self, arg):
		...
		spd=float(arg)
		playerm2g2.SetAutoCameraRotationSpeed(spd)
	...
	def SetMovingSpeed(self, arg):
		"Set walking speed"
		pass
		#chrmgrm2g.SetMovingSpeed(int(arg))
	...
	def SetCoolTime(self):
		"Cooltime toggle"
		flag = playerm2g2.ToggleCoolTime()
		if flag:
	...
	def SetLevelLimit(self):
		"Sets Level limit"
		flag = playerm2g2.ToggleLevelLimit()
		if flag:
	...
	def SetHitEffect(self, arg):
		...
		effectFullPathFileName=self.GetFullPathName(EffectFileName)
		chrmgrm2g.RegisterEffect(chrmgrm2g.EFFECT_HIT, "", effectFullPathFileName)
	...
	def SetStunEffect(self, arg):
		...
		effectFullPathFileName = self.GetFullPathName(effectFileName)
		chrmgrm2g.RegisterEffect(chrmgrm2g.EFFECT_STUN, "Bip01 Head", effectFullPathFileName)
	...
	def SetDustEffect(self, arg):
		...
		effectFullPathFileName = self.GetFullPathName(effectFileName)
		chrmgrm2g.RegisterEffect(chrmgrm2g.EFFECT_DUST, "", effectFullPathFileName)
	...
	def SetSkillGroupFake(self, index):
		"""테스트 코드"""
		pass
		#m2netm2g.SetSkillGroupFake(int(index))
		#self.Print(" SetSkillGroupFake : %d" % int(index))

	def SetEmpireLanguageMode(self, mode):
		m2netm2g.SetEmpireLanguageMode(int(mode))
