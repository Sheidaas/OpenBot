...
class WhisperDialog(ui.ScriptWindow):
	...
	class TextRenderer(ui.Window):
		...
		def OnRender(self):
			...
			chatm2g.RenderWhisper(self.targetName, x, y)
	...
	def SetWhisperDialogSize(self, width, height):
		try:
			...
			if 0 != self.targetName:
				chatm2g.SetWhisperBoxSize(self.targetName, width - 50, height - 90)			
	...
	def OpenWithTarget(self, targetName):
		chatm2g.CreateWhisper(targetName)
		chatm2g.SetWhisperBoxSize(targetName, self.GetWidth() - 60, self.GetHeight() - 90)
	...
	def Close(self):
		chatm2g.ClearWhisper(self.targetName)
		self.titleNameEdit.KillFocus()
		self.chatLine.KillFocus()
		self.Hide()
	...
	def ReportViolentWhisper(self):
		m2netm2g.SendChatPacket("/reportviolentwhisper " + self.targetName)

	def IgnoreTarget(self):
		m2netm2g.SendChatPacket("/ignore " + self.targetName)
	...
	def OnScroll(self):
		chatm2g.SetWhisperPosition(self.targetName, self.scrollBar.GetPos())
